...
for sample in `awk -F"\t" 'NR>1 {print $1}' $tumornormallist | sort | uniq `; do 
	...
	script=scripts/submit_pyclone-vi.sh ## only part II
	scriptsNew=scripts/batches/submit_pyclone-vi.$sample.sh
	cp $script $scriptsNew
	sed -i "s|^sample=.*|sample=$sample|;s|^genome=.*|genome=$genome|;s|^tumor=.*|tumor=$tumor|;s|^normal=.*|normal=$normal|;s|^part=.*|part=$part|" $scriptsNew
